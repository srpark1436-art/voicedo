              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                  ...
                  <span className={`flex items-center gap-1 text-[13.5px] font-medium ${
                    ...
                    {todo.deadline_time && (
                      <span className="text-violet-500 ml-1">
                        {parseInt(todo.deadline_time.split(':')[0]) < 12 ? '오전' : '오후'} {parseInt(todo.deadline_time.split(':')[0]) === 0 ? 12 : parseInt(todo.deadline_time.split(':')[0]) > 12 ? parseInt(todo.deadline_time.split(':')[0]) - 12 : parseInt(todo.deadline_time.split(':')[0])}시{parseInt(todo.deadline_time.split(':')[1]) > 0 ? ` ${parseInt(todo.deadline_time.split(':')[1])}분` : ''}
                      </span>
                    )}
                  </span>
                )}
                {todo.reminder_at && (
                  <span className="flex items-center gap-1 text-[12px] font-medium text-amber-500">
                    ...
                    {format(parseISO(todo.reminder_at), 'M/d H:mm', { locale: ko })}
                  </span>
                )}
              </div>
            </>
